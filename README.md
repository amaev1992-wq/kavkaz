...
## 8. Как подключить backend/CRM к форме

Форма отправляет заявки через `submitLead()` в **`src/services/leads.ts`**.

- Задайте `NEXT_PUBLIC_LEAD_API_ENDPOINT` — форма начнёт слать
  `POST` с JSON-телом `LeadPayload` (имя, телефон, город, участок, формат).
- Формат тела под конкретную CRM (Bitrix24, amoCRM, свой backend)
  адаптируется прямо в `submitLead()`.
- **Пока endpoint не задан, реальной отправки не происходит**: заявка
  логируется в консоль (dev-режим), форма показывает success-состояние
  для проверки UX.
